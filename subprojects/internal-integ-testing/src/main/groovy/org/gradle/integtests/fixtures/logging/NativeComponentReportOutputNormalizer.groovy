...
class NativeComponentReportOutputNormalizer implements OutputNormalizer {
    ...
    String normalize(String commandOutput, ExecutionMetadata executionMetadata) {
        return commandOutput
            ...
            .replaceAll("(build/libs/.+/shared/lib(\\w+))\\.(\\w+)") { it[1] + ".dylib" }
            .replaceAll("(build/libs/.+/static/lib(\\w+))\\.(\\w+)") { it[1] + ".a" }
//            .replaceAll('(build/exe/.+/)(\\w+)') { it[1] + OperatingSystem.current().getExecutableName(it[2]) }
    }
}
